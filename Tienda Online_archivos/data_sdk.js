(()=>{"use strict";
	class Preconditions{static checkArgument(cond,msg,...args){
	...
	static checkEquals(a,b,msg,...args){if(a!==b)throw new Error(msg==null?Preconditions.format("{} != {}",Preconditions.stringify(a),Preconditions.stringify(b)):Preconditions.format(msg,...args))}
	static stringify(x){if(x==null||typeof x==="symbol")return String(x);else try{return JSON.stringify(x)}catch(e){return String(x)}}
	...
	static format(template,...args){let i=0;return template.replace(/\{}/g,(()=>i<args.length?args[i++]:"{}"))}}
	class UnreachableError extends Error{constructor(x){super(`unhandled case: ${JSON.stringify(x)}`)}}
	class OkImpl{get ok(){return true}map(f){return new OkImpl(f(this.value))}mapError(){return this}constructor(value){this.value=value}}
	class ErrImpl{get ok(){return false}map(){return this}mapError(f){return new ErrImpl(f(this.error))}constructor(error){this.error=error}}
	const Result={Ok:OkFactory,Err:ErrFactory};
	function OkFactory(a){return new OkImpl(a)}
	function ErrFactory(x){return new ErrImpl(x)}
	const formattedErrorMessage=opts=>{
		const maybeFullstop=opts.message.endsWith(".")?"":".";return`[${opts.code}]:  ${opts.message}${maybeFullstop}`.trim()
	};
	class CanvaErrorClass extends Error{constructor(opts){opts.code=appSandboxErrorCodeMapper(opts.code);super(formattedErrorMessage(opts));
		this.code=opts.code;
		this.name="CanvaError";
		this.rawMessage=opts.message;
		Object.setPrototypeOf(this,CanvaErrorClass.prototype)}
	}
	function setAppSandboxErrorCodeMapper(mapper){appSandboxErrorCodeMapper=mapper}
	let appSandboxErrorCodeMapper=code=>code;
	class PromiseTimeoutError extends Error{
		constructor(){super("[internal_error] Comms promise timed out.")}}
	const REJECT_ON_NEXT_TICK_DELAY_MS=500;
	class PromiseWithTimeout{reset(newTimeoutMs){
		if(newTimeoutMs)this.timeoutMs=newTimeoutMs;this.setTimeout()}
		resolve(value){clearTimeout(this.timeoutId);this.resolveFn(value)}
		reject(reason){clearTimeout(this.timeoutId);this.rejectFn(reason)}
		promise(){return this.p}
		rejectOnNextTick(){this.timeoutId=setTimeout((()=>{this.rejectFn(new PromiseTimeoutError)}),REJECT_ON_NEXT_TICK_DELAY_MS)}
		setTimeout(){clearTimeout(this.timeoutId);
			this.timeoutId=setTimeout((()=>{this.rejectOnNextTick()}),Math.max(this.timeoutMs-REJECT_ON_NEXT_TICK_DELAY_MS,REJECT_ON_NEXT_TICK_DELAY_MS))}
		constructor(timeoutMs){this.timeoutMs=timeoutMs;
			this.p=new Promise(((resolve,reject)=>{this.resolveFn=resolve;
				this.rejectFn=reject}));
			this.setTimeout()}}
	class MessageBus{constructor(handler,port,errorService){
		...
		this.send=message=>{
			try{
				...
				return Result.Ok()
			}catch(e){this.errorService.errorException(e);
				return Result.Err(e)}
		};
		this.onMessageError=e=>{this.errorService.errorException(e)};
		this.onMessage=({data})=>{
			if(!data){
				this.errorService.error(new CanvaErrorClass({code:"internal_error",message:`missing data in 'MessageEvent'`}));
				return}
			try{
	...
	function uuidv4(rng){
		...
		return`${1e7}-${1e3}-${4e3}-${8e3}-${1e11}`.replace(/[018]/g,(function(c){const cN=Number(c);
			return(cN^randomValues[currentRandomValue++]&15>>cN/4).toString(16)}))
	}

	function getRandomValues(rng,amount){
		if(!rng&&typeof window!=="undefined"&&typeof window.crypto!=="undefined"&&typeof window.crypto.getRandomValues==="function")
	...
	const INITIAL_ACK_TIMEOUT=5e3;
	const SUBSEQUENT_ACK_TIMEOUT=2e4;

	class Client{request(path,payload){
		...
		if(!result.ok){
			this.errorService.errorException(result.error,{errorMessagePrefix:"unable to send request",tags:new Map([["type","request"],["path",path]])});
		requestPromise.reject(result.error)
		}
		return resultPromise;
		async function awaitResult(){pendingRequests.set(requestId,{path,requestPromise});
			try{
				const response=await requestPromise.promise();
				return Result.Ok(response)
			}catch(e){
				if(e instanceof PromiseTimeoutError)
					return Result.Err(new CanvaErrorClass({
						code:"internal_error",message:`Comms promise timed out (${path}).`
					}));
				return Result.Err(e)
			}finally{pendingRequests.delete(requestId)}}}handleReply(msg){
				const{requestId,type}=msg;
				const{path,requestPromise:request}=this.pendingRequests.get(requestId)||{};
				if(!request){
					if(type!=="ack")this.errorService.error(`request has already been handled and resolved or was not sent from this client`,{tags:new Map([["type",type],["requestId",`${requestId}`]])});
					return}
				switch(type){
					case"response":request.resolve(msg.payload);
						return;
					case"ack":request.reset(SUBSEQUENT_ACK_TIMEOUT);
						return;
					case"error":this.errorService.addBreadCrumb({level:"info",category:"sandbox_comms",message:"Error response received",data:{["path"]:path??"unknown"}});
						const{code,message}=msg;request.reject(new CanvaErrorClass({code,message}));
						return;
					default:throw new UnreachableError(msg)}}

		constructor(send,errorService,requestIdGenerator=uuidv4){this.send=send;this.errorService=errorService;
			this.requestIdGenerator=requestIdGenerator;
			this.pendingRequests=new Map}}
	
	const RequestHandlerErrorCodes={internalError:()=>"Something went wrong on our end, if this issue persists please contact us.",noHandlerForPath:path=>`No request handler configured for path: "${path}".`,handlerForPathAlreadyDefined:path=>`Handler for '${path}' is already defined.`};
	class RequestHandler{addErrorObserver(observer){this.errorObservers.add(observer);
		return()=>this.errorObservers.delete(observer)}handle(path,handler){
			if(this.requestHandler.has(path))throw new CanvaErrorClass({code:"internal_error",message:RequestHandlerErrorCodes.handlerForPathAlreadyDefined(path)});
		...
		async handleRequest(body){
			...
			if(!handler){
				...
				if(!result.ok)this.errorService.criticalException(result.error,{errorMessagePrefix:"unable to send error response",tags:new Map([["type","request"],["path",path]])});
				return}this.messenger.sendAck(requestId);
			const intervalId=setInterval((()=>this.messenger.sendAck(requestId)),ACK_INTERVAL);
			let observableError;
			try{
				...
				if(e instanceof CanvaErrorClass){observableError=e;
					if(e.code==="internal_error")this.errorService.errorException(e,{errorMessagePrefix:"Internal error in comms handler",tags:new Map([["type","request"],["path",path]])});
			else{errorCode=e.code;errorMessage=e.rawMessage}
				}else if(
					this.reportNonCanvaErrors)this.errorService.errorException(e,{errorMessagePrefix:"Unexpected error type thrown from comms handler",tags:new Map([["type","request"],["path",path]])});
			...
			for(
				const cb of this.errorObservers)
				try{cb(observableError)
				}catch(e){
					this.errorService.warningException(e,{errorMessagePrefix:"Error executing errorObserver"})}}
	...
	class SandboxCommsBase{
		constructor(port,errorService,reportNonCanvaErrors){
			...
			const handleMessage=message=>{
				switch(message.type){
					case"ack":case"error":case"response":this.client.handleReply(message);
						return;
					case"request":this.requestHandler.handleRequest(message);
						return;
					default:throw new UnreachableError(message)}};
			const messenger=new MangleSafeMessageBus(handleMessage,port,errorService.createChild("bus"));
			this.client=new Client(messenger.sendRequest,errorService.createChild("client"));
			this.requestHandler=new RequestHandler(messenger,errorService.createChild("requestHandler"),reportNonCanvaErrors)}}
	class MangleSafeMessageBus{sendResponse(requestId,payload){
		return this.bus.send({["type"]:"response",["requestId"]:requestId,["payload"]:payload})}
		sendError(requestId,code,message){
			return this.bus.send({["type"]:"error",["requestId"]:requestId,["code"]:code,["message"]:message})}
		sendAck(requestId){
			return this.bus.send({["type"]:"ack",["requestId"]:requestId})}
		constructor(handleMessage,port,errorService){
			this.handleMessage=handleMessage;
			this.sendRequest=(requestId,path,payload)=>this.bus.send({["type"]:"request",["requestId"]:requestId,["path"]:path,["payload"]:payload});
			this.onReceive=message=>{switch(message["type"]){
				case"ack":this.handleMessage({type:"ack",requestId:message["requestId"]});
					return;
				case"error":this.handleMessage({type:"error",requestId:message["requestId"],code:message["code"],message:message["message"]});
					return;
				case"response":this.handleMessage({type:"response",requestId:message["requestId"],payload:message["payload"]});
					return;
				case"request":this.handleMessage({type:"request",requestId:message["requestId"],path:message["path"],payload:message["payload"]});
					return;
				default:throw new UnreachableError(message)}};
			this.bus=new MessageBus({receive:this.onReceive},port,errorService)}}
	const VerifyError={MISSING_DATA:"missing 'data' field in MessageEvent",MISSING_SOURCE:"'source' is missing in MessageEvent data object",INVALID_SOURCE:"invalid source",UNKNOWN_CLIENT_ID:"unknown client id"};
	function createInitialiseMessage(source,clientId){return{["source"]:source,["sandboxCommsSource"]:source,["clientId"]:clientId}}
	function verifyMessage(data,expectedSource,expectedClientId){if(!data)return Result.Err("missing 'data' field in MessageEvent");
		const msg=data;
		const source=msg["sandboxCommsSource"]??msg["source"];if(!source)return Result.Err("'source' is missing in MessageEvent data object");if(source!==expectedSource)return Result.Err("invalid source");
		if(msg["clientId"]!==expectedClientId)
			return Result.Err("unknown client id");
		return Result.Ok()}
	const PARENT_RESPONSE_TIMEOUT_MS=1e3;const MAX_NUM_CONNECTION_ATTEMPTS=4;
	async function connectToParent(errorService,parentOrigin,{parent,addEventListener,removeEventListener}=window,clientId){
		let handshakeTimeout=new PromiseWithTimeout(PARENT_RESPONSE_TIMEOUT_MS);
		const messageHandler=({ports,data})=>{
			const msg=verifyMessage(data,"parent",clientId);
			if(!msg.ok){
				switch(msg.error){
					case VerifyError.INVALID_SOURCE:errorService.info(msg.error,{errorMessagePrefix:"failed to verify message",extra:new Map([["data.source",data?.source]])});
						break;
					case VerifyError.MISSING_DATA:case VerifyError.MISSING_SOURCE:case VerifyError.UNKNOWN_CLIENT_ID:break;
					default:throw new UnreachableError(msg.error)}
				return}
			if(ports.length!==1){errorService.error("invalid number of ports received from the parent");
				return}
			const[port]=ports;handshakeTimeout.resolve(port)};
		...
		const getPortWithRetry=async()=>{
			...
			let count=1;
			while(count<=MAX_NUM_CONNECTION_ATTEMPTS)
				try{port=await handshakeTimeout.promise();
					break
				}catch(e){
					const timeoutWithBackoffMs=count*PARENT_RESPONSE_TIMEOUT_MS;handshakeTimeout=new PromiseWithTimeout(timeoutWithBackoffMs);
					parent.postMessage(initMessage,parentOrigin);
					count++}
			if(port==null)throw new CanvaErrorClass({code:"internal_error",message:`Iframe connection timed out after ${MAX_NUM_CONNECTION_ATTEMPTS} retries`});
			return port};
		try{const port=await getPortWithRetry();
			return Result.Ok(new SandboxCommsBase(port,errorService.createChild("comms"),false))
		}catch(e){
			return Result.Err(e)
		}finally{removeEventListener("message",messageHandler)}}
	function deserializeRequest(message_path,proto,request){
		if(request===undefined)throw new CanvaErrorClass({code:"internal_error",message:`${message_path}: request cannot be undefined.`});
	...
	async function runRequest(comms,messagePath,requestProto,request,responseProto){
		...
		if(!response.ok){
			...
			throw new CanvaErrorClass({code:"internal_error",message:`Something went wrong during request ${messagePath}`})}
		if(responseProto){
			if(response.value===undefined)throw new CanvaErrorClass({code:"internal_error",message:`Expected a response from ${messagePath}, but got nothing`});
			return responseProto.deserialize(response.value)}}
	const MessagePaths={INIT_DATA:"INIT_DATA",DATA_RENDER_READY:"DATA_RENDER_READY"};
	function isPromiseLike(p){return p!=null&&p.then!=null}
	function memoize(fn,{invalidateOnError}={invalidateOnError:false}){
		let promiseDidReject=false;
		let result;
		const memoFn=(...args)=>{Preconditions.checkArgument(args.length===0);
			if(result==null||invalidateOnError&&(!result.ok||promiseDidReject))
				try{promiseDidReject=false;
					result=Result.Ok(fn());
					if(isPromiseLike(result.value))result.value.then(null,(_e=>promiseDidReject=true))
				}catch(e){result=Result.Err(e)}
			if(result.ok)return result.value;
			else throw result.error};
		return memoFn}
	function memoize1(fn){
		const cache=new WeakMap;
		const memoFn=(key,...rest)=>{Preconditions.checkArgument(rest.length===0);
			if(!cache.has(key))cache.set(key,fn(key));
			return cache.get(key)};
		return memoFn}
	const MessageKind={CONCRETE:1,ONEOF:2};
	const FieldLabel={CONSTANT:1,REQUIRED:2,OPTIONAL:3,REPEATED:4,MAP:5};
	const FieldKind={PRIMITIVE:1,MESSAGE:2,ENUM:3};
	const makeType=t=>t;
	const ProtoType={STRING:makeType({typeOfValue:"string"}),
		BOOLEAN:makeType({typeOfValue:"boolean",
			defaultValue:false,
			fixedSize:1}),
		DOUBLE:makeType({
			typeOfValue:"number",
			defaultValue:0,
			fixedSize:8}),
		INT32:makeType({typeOfValue:"number",
			defaultValue:0}),
		INT64:makeType({typeOfValue:"number",
			defaultValue:0}),
	...
	function constantField(params){
		...
		return{tag,fieldLabel:FieldLabel.CONSTANT,jsonFullKey:jsonFullKeyOrJsonMiniKey===DISCRIMINATOR_JSON_MINI_KEY?undefined:jsonFullKeyOrJsonMiniKey,jsonFullValue,value,defaults,typeOfValue:"string"}}
	function requiredField(valueType,jsonFullKey,tag,def){
		return{tag,fieldLabel:FieldLabel.REQUIRED,jsonFullKey,default:def!=null?def:valueType.defaultValue,defaultValue:valueType.defaultValue,typeOfValue:valueType.typeOfValue}}
	function optionalField(valueType,jsonFullKey,tag){
		return{tag,fieldLabel:FieldLabel.OPTIONAL,jsonFullKey,defaultValue:valueType.defaultValue,typeOfValue:valueType.typeOfValue}}
	function repeatedField(valueType,jsonFullKey,tag){
		return{tag,fieldLabel:FieldLabel.REPEATED,jsonFullKey,typeOfValue:valueType.typeOfValue}}
	function mapField(keyType,valueType){
		return(tagOrJsonFullKey,tagOrObj,maybeObj)=>{
			...
			return{fieldLabel:FieldLabel.MAP,tag,jsonFullKey,obj,typeOfKey:keyType.typeOfValue,typeOfValue}}}
	class Proto{
		...
		static requiredObject(tagOrJsonFullKey,tagOrObj,maybeObj){
			const{tag,jsonFullKey,other1:obj}=getOptions(tagOrJsonFullKey,tagOrObj,maybeObj);
			return{tag,fieldLabel:FieldLabel.REQUIRED,jsonFullKey,obj,typeOfValue:"object"}}
		static optionalObject(tagOrJsonFullKey,tagOrObj,maybeObj){
			const{tag,jsonFullKey,other1:obj}=getOptions(tagOrJsonFullKey,tagOrObj,maybeObj);
			return{tag,fieldLabel:FieldLabel.OPTIONAL,jsonFullKey,obj,typeOfValue:"object"}}
		static repeatedObject(tagOrJsonFullKey,tagOrObj,maybeObj){
			const{tag,jsonFullKey,other1:obj}=getOptions(tagOrJsonFullKey,tagOrObj,maybeObj);
			return{tag,fieldLabel:FieldLabel.REPEATED,jsonFullKey,obj,typeOfValue:"object"}}
		static requiredStringEnum(tagOrJsonFullKey,tagOrObj,objOrDef,maybeDef){
			...
			return{tag,fieldLabel:FieldLabel.REQUIRED,jsonFullKey,obj,default:def,typeOfValue:"string"}}
		static optionalStringEnum(tagOrJsonFullKey,tagOrObj,maybeObj){
			const{tag,jsonFullKey,other1:obj}=getOptions(tagOrJsonFullKey,tagOrObj,maybeObj);
			return{tag,fieldLabel:FieldLabel.OPTIONAL,jsonFullKey,obj,typeOfValue:"string"}}
		static repeatedStringEnum(tagOrJsonFullKey,tagOrObj,maybeObj){
			const{tag,jsonFullKey,other1:obj}=getOptions(tagOrJsonFullKey,tagOrObj,maybeObj);
			return{tag,fieldLabel:FieldLabel.REPEATED,jsonFullKey,obj,typeOfValue:"string"}}
		static createMessage(schema,options={}){
			const init=memoize((()=>{
				...
				for(
					const name of fieldNames){
					const config=fields[name];
					switch(config.fieldLabel){
						case FieldLabel.CONSTANT:constantsByTag[config.tag]={...config,name};
							break;
						case FieldLabel.REQUIRED:case FieldLabel.OPTIONAL:case FieldLabel.REPEATED:case FieldLabel.MAP:fieldsByTag[config.tag]={...config,name};
							break;
						default:throw new UnreachableError(config)}}
				return{kind:MessageKind.CONCRETE,fields,fieldMetadata:deriveFieldMetadata(fields,options.dualDeserializationConfig),fieldsByTag,constantsByTag}}));
			class Message{
				...
				static serializeValue(value,metadatum,path){
					...
					if(field.typeOfValue==="Uint8Array")
				...
				static deserializeWithPath(o,path){
					...
					for(
						const metadatum of fieldMetadata){
						const{config:field,name:fieldName,primaryJsonKey,secondaryJsonKey}=metadatum;
						let jsonKey=primaryJsonKey;
						let value=o[jsonKey];
						if(value==null&&secondaryJsonKey!=null&&o[secondaryJsonKey]!=null){jsonKey=secondaryJsonKey;value=o[jsonKey]}
						switch(field.fieldLabel){
							case FieldLabel.OPTIONAL:if(value==null){result[fieldName]=undefined;
								break
							...
							case FieldLabel.REQUIRED:
								if(value==null&&field.defaultValue!=null){result[fieldName]=field.defaultValue;
									break
...
constructor(persistenceService,connectToParent,createDataClientComms,errorService){
	...
	this.lastKnownData=[];
	this.pollingState={pollingInterval:null,consecutiveErrorCount:0,hasTerminalErrors:false,pollingIntervalMs:DEFAULT_POLLING_INTERVAL_MS,lastReadTimestamp:0}}}
function getErrorService(){
	const telemetry=window.telemetrySdk?.errorService;return telemetry?.createChild("Data")??new ConsoleErrorService("Data")}window.dataSdk=new CodeletDataSdk(new FetchPersistenceService,connectToParent,(comms=>new DataClientComms(comms)),getErrorService())})();
