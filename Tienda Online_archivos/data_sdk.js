(() => {
            ...
            switch (field.fieldLabel) {
              ...
              case FieldLabel.CONSTANT: {
                ...
                throw new TypeError(`Expected value ${expectedValues(valueEncodings)}
									...
									found: ${JSON.stringify(value)} ${pathTrace(path)}`);
              }
              case FieldLabel.REPEATED: {
                if (value == null) {
                  result[fieldName] = [];
  ...
  const InitDataRequest = Proto.createMessage(() => ({}));
  const WebsiteContext = Proto.createMessage(() => ({
    deploymentId: Proto.requiredString(1),
    accessToken: Proto.requiredString(2),
  }));
  const InitDataResponse = Proto.createMessage(
    () => ({
      documentId: Proto.optionalString("documentId", 1),
      elementId: Proto.optionalString("elementId", 2),
      websiteContext: Proto.optionalObject("websiteContext", 3, WebsiteContext),
      isPreview: Proto.optionalBoolean("isPreview", 4),
    }),
    {
      dualDeserializationConfig:
  ...
  const RenderReadyRequest = Proto.createMessage(() => ({}));
  const RenderReadyResponse = Proto.createMessage(() => ({}));
  const DATA_CLIENT_ID = "canva-code-data-sdk";
  function setupDataRequestHandlers(comms, handler) {
    handleMessageFromHost(
      comms,
      MessagePaths.INIT_DATA,
      InitDataRequest,
      (request) => handler.handleInitData(request),
      InitDataResponse,
    );
    handleMessageFromHost(
      comms,
      MessagePaths.DATA_RENDER_READY,
      RenderReadyRequest,
      (request) => handler.handleRenderReady(request),
      RenderReadyResponse,
    );
  }
  class DataClientComms {
    async sendInitData(request) {
      const result = await this.comms.request(
        MessagePaths.INIT_DATA,
        InitDataRequest.serialize(request),
      );
      const value = extractValue(result, MessagePaths.INIT_DATA);
      return InitDataResponse.deserialize(value);
    }
    async sendRenderReady(request) {
      const result = await this.comms.request(
        MessagePaths.DATA_RENDER_READY,
        RenderReadyRequest.serialize(request),
      );
      const value = extractValue(result, MessagePaths.DATA_RENDER_READY);
      return RenderReadyResponse.deserialize(value);
    }
    constructor(comms) {
      this.comms = comms;
    }
  ...
  function createEndpointUrl(path, baseUrl) {
    ...
    try {
      ...
      normalizedBase = url.toString();
    } catch {
      normalizedBase = urlString.split("#")[0].split("?")[0];
    }
  ...
  function createEndpointUrlWithParams(path, params, baseUrl) {
    const endpointUrl = createEndpointUrl(path, baseUrl);
  ...
  const CodeletSdkSubmission = Proto.createMessage(() => ({
    id: Proto.requiredString(1),
    data: Proto.requiredString(2),
  }));
  const CreateCodeletSdkSubmissionRequest = Proto.createMessage(() => ({
    data: Proto.requiredString(1),
    documentId: Proto.optionalString(2),
    elementId: Proto.optionalString(3),
    websiteDeploymentId: Proto.optionalString(4),
    websiteAccessToken: Proto.optionalString(5),
  }));
  const CreateCodeletSdkSubmissionResponse = Proto.createMessage(() => ({
    submission: Proto.optionalObject(1, CodeletSdkSubmission),
  }));
  const FindCodeletSdkSubmissionsRequest = Proto.createMessage(() => ({
    documentId: Proto.optionalString(1),
    elementId: Proto.optionalString(2),
    websiteDeploymentId: Proto.optionalString(3),
    websiteAccessToken: Proto.optionalString(4),
  }));
  const FindCodeletSdkSubmissionsResponse = Proto.createMessage(() => ({
    enrichedSubmissions: Proto.repeatedObject(2, CodeletSdkSubmission),
    pollEveryMs: Proto.optionalInt32(3),
  }));
  const UpdateCodeletSdkSubmissionRequest = Proto.createMessage(() => ({
    submissionId: Proto.requiredString(1),
    data: Proto.requiredString(2),
    documentId: Proto.optionalString(3),
    elementId: Proto.optionalString(4),
    websiteDeploymentId: Proto.optionalString(5),
    websiteAccessToken: Proto.optionalString(6),
  }));
  const UpdateCodeletSdkSubmissionResponse = Proto.createMessage(() => ({}));
  const DeleteCodeletSdkSubmissionRequest = Proto.createMessage(() => ({
    submissionId: Proto.requiredString(1),
    documentId: Proto.optionalString(2),
    elementId: Proto.optionalString(3),
    websiteDeploymentId: Proto.optionalString(4),
  ...
  const DeleteCodeletSdkSubmissionResponse = Proto.createMessage(() => ({}));
  ...
  class FetchPersistenceService {
    checkRateLimit() {
      ...
      if (timeSinceLastMutation < this.rateLimitMs) {
        ...
        if (this.consecutiveRateLimitBreaches >= this.rateLimitThreshold) {
          ...
          throw new HttpError(
            `Rate limit exceeded. Please wait ${waitTime}ms before making another mutation request.`,
            429,
            "Too Many Requests",
          );
        }
    ...
    async create(record, documentId, elementId, websiteContext) {
      this.checkRateLimit();
      const request = new CreateCodeletSdkSubmissionRequest({
      ...
      const response = await fetch(createEndpointUrl("/data/create"), {
        method: "POST",
        headers: { ["Content-Type"]: "application/json" },
        body: JSON.stringify(
          CreateCodeletSdkSubmissionRequest.serialize(request),
        ),
      });
      if (!response.ok)
        throw new HttpError(
      ...
      const responseProto =
        CreateCodeletSdkSubmissionResponse.deserialize(responseData);
      if (!responseProto?.submission?.id)
        throw new Error(
          "Failed to create record: backend did not return an id",
        );
      return { ...record, __backendId: responseProto.submission.id };
    }
    async update(record, documentId, elementId, websiteContext) {
      this.checkRateLimit();
      const cleanRecord = { ...record };
      delete cleanRecord.__backendId;
      const request = new UpdateCodeletSdkSubmissionRequest({
      ...
      const response = await fetch(createEndpointUrl("/data/update"), {
        method: "POST",
        headers: { ["Content-Type"]: "application/json" },
        body: JSON.stringify(
          UpdateCodeletSdkSubmissionRequest.serialize(request),
        ),
      });
      if (!response.ok)
        throw new HttpError(
      ...
      const responseData = await response.json();
      UpdateCodeletSdkSubmissionResponse.deserialize(responseData);
      return record;
    }
    async delete(id, documentId, elementId, websiteContext) {
      const request = new DeleteCodeletSdkSubmissionRequest({
        submissionId: id,
        documentId,
        elementId,
        websiteDeploymentId: websiteContext?.deploymentId,
      ...
      const response = await fetch(createEndpointUrl("/data/remove"), {
        method: "POST",
        headers: { ["Content-Type"]: "application/json" },
        body: JSON.stringify(
          DeleteCodeletSdkSubmissionRequest.serialize(request),
        ),
      });
      if (!response.ok)
        throw new HttpError(
      ...
      const responseData = await response.json();
      DeleteCodeletSdkSubmissionResponse.deserialize(responseData);
    }
    async read(documentId, elementId, websiteContext) {
      const response = await fetch(
        createEndpointUrlWithParams("/data/getAll", {
          document_id: documentId,
          element_id: elementId,
          website_deployment_id: websiteContext?.deploymentId,
          website_access_token: websiteContext?.accessToken,
        }),
        { method: "GET", headers: { ["Content-Type"]: "application/json" } },
      );
      if (!response.ok)
        throw new HttpError(
          `Failed to read record: ${response.status} ${response.statusText}`,
          response.status,
          response.statusText,
        );
      const responseData = await response.json();
      const submissionData =
        FindCodeletSdkSubmissionsResponse.deserialize(responseData);
      const deserializedSubmissions = [];
      for (const submission of submissionData.enrichedSubmissions)
        try {
          const id = submission.id;
          const parsed = JSON.parse(submission.data);
          deserializedSubmissions.push({ ...parsed, __backendId: id });
        } catch (error) {
          throw new Error(
            `Failed to parse submission: ${submission}, ${error}`,
          );
        }
      return {
    ...
    constructor(
      rateLimitMs = MUTATION_RATE_LIMIT_MS,
  ...
  class CodeletDataSdk {
    async setupCommsClient() {
      ...
      if (!result.ok)
        return SdkResult.error(
          new Error("Failed to establish connection to parent"),
        );
      ...
      return SdkResult.ok(commsClient);
    }
    async create(record) {
      try {
        if (!this.dataHandler)
          return SdkResult.error(
            new Error(
              "SDK must be initialized with init() before calling create()",
            ),
          );
        await this.persistenceService.create(
          record,
          this.documentId,
          this.elementId,
          this.websiteContext,
        ...
        return readResult.isError
          ? SdkResult.error(readResult.error)
          : SdkResult.ok(undefined);
      } catch (e) {
        const error = e instanceof Error ? e : new Error(String(e));
        return SdkResult.error(error);
      }
    }
    async update(record) {
      try {
        if (!this.dataHandler)
          return SdkResult.error(
            new Error(
              "SDK must be initialized with init() before calling update()",
            ),
          );
    ...
    async delete(record) {
      try {
        if (!this.dataHandler)
          return SdkResult.error(
            new Error(
              "SDK must be initialized with init() before calling delete()",
            ),
          );
    ...
    async read() {
      try {
        if (!this.dataHandler)
          return SdkResult.error(
            new Error(
              "SDK must be initialized with init() before calling read()",
            ),
          );
        this.pollingState.lastReadTimestamp = Date.now();
        const { submissions, pollingRateMs } =
          await this.persistenceService.read(
            this.documentId,
            this.elementId,
            this.websiteContext,
          );
        ...
        return SdkResult.ok(submissions);
      } catch (e) {
        const error = e instanceof Error ? e : new Error(String(e));
        return SdkResult.error(error);
      }
    ...
    startPolling() {
      if (this.pollingState.hasTerminalErrors) {
        this.errorService.warning(
          "Not starting polling because polling was previously stopped due to errors",
        );
        return;
      }
      this.stopPolling();
      this.pollingState.pollingInterval = setInterval(async () => {
        try {
          const timeSinceLastRead =
            Date.now() - this.pollingState.lastReadTimestamp;
          if (timeSinceLastRead < this.pollingState.pollingIntervalMs) return;
          const result = await this.read();
          if (result.isOk) this.resetErrorCount();
          else if (this.shouldStopPollingOnError(result.error))
            this.stopPolling();
        } catch (error) {
    ...
    shouldStopPollingOnError(error) {
      ...
      if (error instanceof HttpError) {
        if (TERMINAL_POLLING_ERROR_CODES.includes(error.status)) {
          this.errorService.error(
            `Stopping polling due to status ${error.status}: ${error.message}`,
          );
      ...
      if (this.pollingState.consecutiveErrorCount >= 5) {
        this.errorService.error(
          new Error(
            `Stopping polling after ${this.pollingState.consecutiveErrorCount} consecutive errors.`,
            { cause: error },
          ),
        );
        this.pollingState.hasTerminalErrors = true;
      ...
      this.errorService.warning(
        new Error(
          `Polling error ${this.pollingState.consecutiveErrorCount}/5`,
          { cause: error },
        ),
      );
      return false;
    ...
    async triggerInitialReadAndPreviewReady(isPreview) {
      try {
        ...
        if (!result.isOk)
          this.errorService.error(
            new Error("Failed to initialize with data", {
              cause: result.error,
            }),
          );
      } finally {
        if (isPreview)
          this.commsClient?.sendRenderReady(new RenderReadyRequest());
      }
    }
    async init(handler) {
      try {
        ...
        if (setupResult?.isError) {
          this.errorService.error(
            new Error(
              "Failed to establish connection to parent & initialize Data SDK",
              { cause: setupResult.error },
            ),
          );
          return SdkResult.error(setupResult.error);
        }
        ...
        this.errorService.info("Data SDK initialized");
        if (initRes.isPreview && !(initRes.documentId && initRes.elementId)) {
          commsClient.sendRenderReady(new RenderReadyRequest());
  ...
  function getErrorService() {
    ...
    return telemetry?.createChild("Data") ?? new ConsoleErrorService("Data");
  }
  window.dataSdk = new CodeletDataSdk(
    new FetchPersistenceService(),
    connectToParent,
    (comms) => new DataClientComms(comms),
    getErrorService(),
  );
})();
